...
import logging
logger = logging.getLogger('clipmatch.features')
...
# ── Fast Hamming helpers (avoid imagehash object creation in hot loops) ──

def hex_to_int(hex_str: str) -> int:
    """Convert a hex hash string to a 64-bit integer for fast bitwise ops.
    
    Masks to 64 bits to handle hashes of any size (8×8, 16×16, etc.)
    stored in the database. This prevents numpy int64 overflow errors.
    """
    try:
        return int(hex_str, 16) & 0xFFFFFFFFFFFFFFFF  # Mask to 64 bits
    except (ValueError, TypeError):
        return 0


def fast_hamming(int1: int, int2: int) -> int:
    """Hamming distance via XOR + popcount. ~50× faster than imagehash parsing."""
...
class FeatureExtractor:
    ...
    def extract_dual_hash(self, frame: np.ndarray) -> Dict[str, str]:
        ...
        try:
            ...
            logger.warning("pHash extraction failed: %s", e)
            features['phash'] = None
        ...
        try:
            ...
            logger.warning("dHash extraction failed: %s", e)
            features['dhash'] = None
    ...
    def extract_features(self, frame: np.ndarray) -> Dict[str, str]:
        """
        Extract all features (pHash + dHash + wHash) from a frame in a single pass.
        
        Converts numpy→PIL once and computes all three hashes from the same image.
        
        Args:
            frame: Frame as numpy array (already preprocessed)
            
        Returns:
            Dict with 'phash', 'dhash', and 'whash' as hex strings
        """
        # ── Single PIL conversion for all hashes ──
        if frame.dtype != np.uint8:
            frame = (frame * 255).astype(np.uint8) if frame.max() <= 1 else frame.astype(np.uint8)
        ...
        try:
            ...
            logger.warning("wHash extraction failed: %s", e)
            features['whash'] = None
    ...
    def compute_hash_distance(self, hash1: str, hash2: str) -> int:
        ...
        Uses fast integer XOR + popcount instead of imagehash object creation.
        ...
        """
        try:
            return fast_hamming(hex_to_int(hash1), hex_to_int(hash2))
        except Exception as e:
            logger.warning("Error computing hash distance: %s", e)
