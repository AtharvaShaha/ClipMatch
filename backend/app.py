...
_log_level = logging.DEBUG if os.environ.get('CLIPMATCH_DEBUG') else logging.INFO
logging.basicConfig(
    level=_log_level,
...
def create_app():
    ...
    try:
        ...
        if indexed_count == 0:
            ...
            if result['success']:
                print(f"[Init] [OK] Auto-indexed {len(result['indexed'])} video(s)")
                for video_info in result['indexed']:
                    print(f"     - {video_info['filename']}")
            else:
                print(f"[Init] No reference videos to index or all failed")
        else:
            print(f"[Init] [OK] Found {indexed_count} already indexed video(s)")
...
if __name__ == '__main__':
    ...
    - Uploads:    {UPLOADS_DIR}
    - Database:   {DATA_DIR / 'clipmatch.db'}
    """
    print(banner)
